...
- **Phase 16 — AI Narrative & Production Pipeline (Claude → GPT-4o → ElevenLabs + MiniMax H3).** Docs updated (`docs/architecture.md` §12, `docs/product_roadmap.md` Phase 16). **16.1–16.5 all IMPLEMENTED**: Claude narrative engine; GPT-4o `ProductionManifest` structurer; MiniMax H3-Max Turbo (15s/1080P/native audio, ADR-002); ElevenLabs scene narration (E2E 12/12); manifest persistence + scene-video consumption. Full pipeline: idea → Claude cinematic story → GPT-4o manifest → per-scene MiniMax video + ElevenLabs VO → Movie Builder. Credentials staged in `cred/fal_env.txt` (gitignored): `CLAUDE_API`, `GPT40_API` — server-only, never `NEXT_PUBLIC_*`; ElevenLabs reuses `11_LABS`/`ELEVENLABS_API_KEY`.
