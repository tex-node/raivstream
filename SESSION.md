...
- **Phase 16 — AI Narrative & Production Pipeline (Claude → GPT-4o → ElevenLabs + MiniMax H3).** Docs updated (`docs/architecture.md` §12, `docs/product_roadmap.md` Phase 16). **16.1 IMPLEMENTED** (Claude narrative engine). **16.2 IMPLEMENTED** (GPT-4o `ProductionManifest` structurer + `story.structureProductionManifest`). **16.3 IMPLEMENTED** (MiniMax H3-Max Turbo: duration ≤15s, resolution `480P|768P|1080P`, native audio verified at 1080×1920@24fps aac; ADR-002 = fal queue; `GenerationJob.resolution`). **16.4 IMPLEMENTED** (scene narration: shared `generateSpeechForCue` + `story.generateSceneNarration` → NARRATION `AudioCue` on the audio plan → mixer). 16.5 not started. Credentials staged in `cred/fal_env.txt` (gitignored): `CLAUDE_API`, `GPT40_API` — server-only, never `NEXT_PUBLIC_*`; ElevenLabs reuses `11_LABS`/`ELEVENLABS_API_KEY`.
