    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-[rgba(233,233,237,0.1)] bg-[rgba(233,233,237,0.03)] p-6">
        ...
        <p className="mt-2 text-sm text-[var(--noc-t4)]">
          {wantsName
            ? `I won’t invent your brand — tell me the name and I’ll use it.`
            : `I won’t invent your real ${contextType.toLowerCase()}. Share the real one or tell me to design a fictional one.`}
        </p>
        ...
        {wantsName ? (
          <div className="mt-5 flex flex-col gap-2 sm:flex-row">
            ...
            <button
              type="button"
              disabled={busy || describe.trim().length < 1}
              onClick={() => onResolve({ kind: 'describe', text: `The brand is called ${describe.trim()}` })}
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
              <label className={`cursor-pointer rounded-xl bg-[var(--noc-purple)] px-4 py-2 text-sm font-black text-[#0B0D12] ${busy ? 'opacity-50' : ''}`}>
                <input
                  type="file"
                  accept="image/*"
                  className="hidden"
