...
export default function StoryWorkspacePage() {
  ...
  if (!isLoaded) {
    return <Shell backHref="/story-playground" title="Loading…" activeTab="home" projectId={projectId}><main className="mx-auto max-w-6xl px-4 py-12 font-bold text-[var(--noc-t4)]">Loading workspace...</main></Shell>;
  }
  if (!isSignedIn) {
  ...
  if (workspace.error) {
    return <Shell backHref="/story-playground" title="Story Playground" activeTab="home" projectId={projectId}><main className="mx-auto max-w-3xl px-4 py-12"><h1 className="text-3xl font-black">We could not open this story.</h1><p className="mt-3 font-bold text-[var(--noc-t4)]">{workspace.error.message}</p><Link href="/story-playground" className="mt-5 inline-block rounded-xl bg-[var(--noc-page)] px-5 py-3 font-black text-white">Back to My Stories</Link></main></Shell>;
  }
  ...
  const renderHero = () => (
    <section className="rounded-3xl bg-[var(--noc-page)] p-5 text-white md:p-7">
    ...
    return (
      <section className="mt-4 rounded-2xl border border-[rgba(79,139,214,0.25)] bg-[rgba(79,139,214,0.10)] p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-black uppercase text-[var(--noc-purple)]">Academy Assignment</p>
            <h2 className="text-xl font-black">{assignment?.title ?? 'Loading assignment...'}</h2>
            {assignment?.brief && <p className="mt-1 text-sm font-bold text-[var(--noc-t4)]">{assignment.brief}</p>}
          </div>
          <div className="flex flex-wrap gap-2">
            {assignment?.assignmentId && <Link href={`/academy/classes/${assignment.classId}/assignments/${assignment.assignmentId}`} className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.10)] px-4 py-3 text-sm font-black text-[var(--noc-t1)]">View Instructions</Link>}
            <button onClick={() => setMessage('Progress saved in this Story Workspace.')} className="rounded-xl bg-[linear-gradient(90deg,#d946a8,#b25ad9)] px-4 py-3 text-sm font-black text-[var(--noc-t1)]">Save Progress</button>
      ...
      <div className="grid gap-3 md:grid-cols-5">
        {[
          ...
          <div key={name} className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4">
            <p className="text-xs font-black uppercase tracking-wide text-[var(--noc-t4)]">{name}</p>
            <p className="mt-2 text-2xl font-black">{value}</p>
          </div>
        ))}
      </div>
      <div className="grid gap-3 md:grid-cols-4">
        <button onClick={() => continueStory.mutate({ projectId })} className="rounded-2xl bg-[var(--noc-page)] px-5 py-4 text-left font-black text-white">{isR16 ? 'Keep Writing' : 'Continue Writing'}</button>
        ...
        <button onClick={() => chooseTab('scenes')} className="rounded-2xl bg-[linear-gradient(90deg,#d946a8,#b25ad9)] px-5 py-4 text-left font-black text-[var(--noc-t1)]">{isR16 ? 'Add Pictures' : 'Add Pictures'}</button>
        {isR16
  ...
  const renderStory = () => (
    <section className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-sm font-black uppercase text-[var(--noc-purple)]">{isR16 ? 'My Story' : 'Story'}</p>
          <h2 className="text-3xl font-black">{project.title}</h2>
          <p className="mt-2 font-semibold text-[var(--noc-t4)]">{project.synopsis ?? project.logline ?? project.originalIdea}</p>
        </div>
        <button onClick={() => continueStory.mutate({ projectId })} className="rounded-xl bg-[var(--noc-page)] px-4 py-3 font-black text-white">{isR16 ? 'Keep Writing' : 'Continue Story'}</button>
      </div>
      <div className="mt-5 grid gap-4">
        {chapters.map((chapter: any) => (
          <article key={chapter.id} className="rounded-2xl bg-[var(--noc-page)] p-4">
            <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Chapter {chapter.chapterNumber}</p>
            <h3 className="mt-1 text-xl font-black">{chapter.title}</h3>
            <p className="mt-2 font-semibold text-[var(--noc-t4)]">{chapter.body}</p>
          </article>
        ))}
      </div>
      {project.storyDna && (
        <div className="mt-5 rounded-2xl bg-[rgba(79,139,214,0.10)] p-4">
          <p className="text-sm font-black uppercase text-[var(--noc-purple)]">{isR16 ? 'What the story is about' : 'Story DNA Summary'}</p>
          <div className="mt-3 grid gap-2 text-sm font-bold text-[var(--noc-t1)] md:grid-cols-3">
  ...
  const renderCharacters = () => (
    <section className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-5">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-sm font-black uppercase text-[var(--noc-blue)]">{isR16 ? 'Characters' : 'Character Director'}</p>
          <h2 className="text-3xl font-black">{isR16 ? 'My Characters' : 'Characters'}</h2>
        </div>
        <button onClick={() => openCharacter()} className="inline-flex items-center gap-2 rounded-xl bg-[linear-gradient(90deg,#d946a8,#b25ad9)] px-4 py-3 font-black text-[var(--noc-t1)]"><Plus size={18} />{isR16 ? 'Add Friend' : 'Add Character'}</button>
      </div>
      {characters.length === 0 ? <div className="rounded-xl border border-dashed border-[rgba(233,233,237,0.10)] p-8 text-center font-bold text-[var(--noc-t4)]">No characters yet.</div> : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {characters.map((character) => (
            <button key={character.id} onClick={() => openCharacter(character)} className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4 text-left hover:border-[var(--noc-purple)]">
              <UserRound className="text-[var(--noc-blue)]" />
              <h3 className="mt-3 text-xl font-black">{character.name}</h3>
              <p className="mt-1 text-sm font-bold text-[var(--noc-t4)]">{[character.ageDescription, character.gender, character.species].filter(Boolean).join(' ') || character.role || 'Story character'}</p>
              <p className="mt-3 line-clamp-3 text-sm font-semibold text-[var(--noc-t4)]">{character.visualDescription}</p>
              <div className="mt-3 flex flex-wrap gap-2">{(character.personalityTraits ?? []).slice(0, 4).map((trait) => <span key={trait} className="rounded-full bg-[rgba(79,139,214,0.12)] px-3 py-1 text-xs font-black text-[var(--noc-blue)]">{label(trait)}</span>)}</div>
            </button>
          ))}
        </div>
      )}
            ...
            return (
              <article key={scene.id} className="overflow-hidden rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)]">
                <div className="aspect-[9/12] bg-[rgba(233,233,237,0.06)] bg-cover bg-center" style={active?.assetUrl ? { backgroundImage: `url("${active.assetUrl}")` } : undefined}>
                  {!active?.assetUrl && <div className="flex h-full items-center justify-center text-[var(--noc-purple)]"><Camera size={42} /></div>}
                </div>
                <div className="p-4">
                  <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Scene {scene.orderIndex}</p>
                  ...
                  {[scene.locationType, scene.indoorOutdoor, scene.timeOfDay ? label(scene.timeOfDay) : null, scene.mood].filter(Boolean).length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1">
                      {[scene.locationType, scene.indoorOutdoor, scene.timeOfDay ? label(scene.timeOfDay) : null, scene.mood].filter(Boolean).map((tag) => (
                        <span key={tag} className="rounded-full border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.06)] px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-[var(--noc-t4)]">{tag}</span>
                      ))}
                    </div>
                  )}
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    ...
                    <button onClick={() => { chooseTab('assets'); setMessage(scene.title); }} className="rounded-xl bg-[rgba(79,139,214,0.10)] border border-[rgba(79,139,214,0.20)] px-3 py-2 text-sm font-black text-[var(--noc-purple)]">{isR16 ? 'See Pictures' : 'Open Assets'}</button>
                  </div>
                </div>
              </article>
            );
  ...
  const renderAssets = () => (
    <section className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-5">
      <div className="mb-4">
        <p className="text-sm font-black uppercase text-[var(--noc-magenta)]">{isR16 ? 'Pictures' : 'Asset Manager'}</p>
      ...
      <div className="space-y-6">
        {scenes.map((scene) => {
          ...
          return (
            <div key={scene.id} className="rounded-2xl bg-[var(--noc-page)] p-4">
              ...
              {assets.length === 0 ? <div className="rounded-xl border border-dashed border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-6 text-center font-bold text-[var(--noc-t4)]">No pictures yet.</div> : (
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {assets.map((asset) => {
                    ...
                    return (
                      <article key={asset.id} className={`overflow-hidden rounded-2xl border bg-[rgba(233,233,237,0.04)] ${isActive ? 'border-[var(--noc-purple)]' : 'border-[rgba(233,233,237,0.08)]'}`}>
                        ...
                        <div className="p-3">
                          <div className="mb-2 flex flex-wrap gap-2 text-xs font-black">
                            ...
                            {isActive && <span className="rounded-full bg-[rgba(79,139,214,0.12)] px-2 py-1 text-[var(--noc-blue)]">{isR16 ? 'In Book' : 'Active'}</span>}
                            {asset.isLatest && <span className="rounded-full bg-[rgba(233,233,237,0.06)] px-2 py-1">Latest</span>}
                            {asset.isFavorite && <span className="rounded-full bg-[rgba(217,70,168,0.12)] px-2 py-1 text-[var(--noc-magenta)]">Favorite</span>}
                            {!isR16 && <span className="rounded-full bg-[rgba(79,139,214,0.10)] px-2 py-1 text-[var(--noc-purple)]">{criticStatusLabel(asset)}</span>}
                          </div>
                          <p className="text-xs font-bold text-[var(--noc-t4)]">{dateLabel(asset.createdAt)} {rating ? `| Rating ${rating > 0 ? '+' : ''}${rating}` : ''}</p>
                          {canUseTechnical && <p className="mt-1 text-xs font-semibold text-[var(--noc-t4)]">{asset.provider} | {asset.model} | {asset.width}x{asset.height}</p>}
                          {!isR16 && (
                            <div className="mt-3 rounded-xl bg-[rgba(79,139,214,0.10)] p-3 text-xs font-semibold text-[var(--noc-t4)]">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <span className="font-black text-[var(--noc-t1)]">Creative Critic</span>
                              ...
                              {criticRun?.issues?.length ? <p className="mt-2 line-clamp-2 text-[var(--noc-magenta)]">Issue: {criticRun.issues[0].description}</p> : null}
                          ...
                          <div className="mt-3 grid gap-2">
                            ...
                            <button onClick={() => favoriteAsset.mutate({ projectId, assetId: asset.id, isFavorite: !asset.isFavorite })} className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.10)] px-3 py-2 text-sm font-black text-[var(--noc-t1)]"><Heart className="mr-1 inline" size={16} />{asset.isFavorite ? 'Unfavorite' : 'Favorite'}</button>
                            {!isR16 && <button onClick={() => runCreativeCritic.mutate({ projectId, sceneId: scene.id, assetId: asset.id })} disabled={runCreativeCritic.isPending} className="rounded-xl bg-[rgba(79,139,214,0.10)] px-3 py-2 text-sm font-black text-[var(--noc-purple)]"><RefreshCw className="mr-1 inline" size={16} />Review Again</button>}
                            {!isR16 && criticRun?.recommendation && criticRun.recommendation !== 'APPROVE' && (
                              <button onClick={() => regenerateFromCritic.mutate({ projectId, criticRunId: criticRun.id, model: 'FLUX2' })} disabled={regenerateFromCritic.isPending} className="rounded-xl bg-[linear-gradient(90deg,#d946a8,#b25ad9)] px-3 py-2 text-sm font-black text-[var(--noc-t1)]">Improve and Regenerate</button>
                            )}
                            {!isR16 && <button onClick={() => approveSceneAsset.mutate({ projectId, assetId: asset.id })} disabled={approveSceneAsset.isPending} className="rounded-xl bg-[rgba(79,139,214,0.12)] px-3 py-2 text-sm font-black text-[var(--noc-blue)]"><CheckCircle2 className="mr-1 inline" size={16} />Approve</button>}
                            {!isR16 && <button onClick={() => rejectSceneAsset.mutate({ projectId, assetId: asset.id })} disabled={rejectSceneAsset.isPending} className="rounded-xl border border-[rgba(217,70,168,0.20)] bg-[rgba(217,70,168,0.08)] px-3 py-2 text-sm font-black text-[var(--noc-magenta)]"><X className="mr-1 inline" size={16} />Reject</button>}
                            {!isR16 && (
                              <div className="grid grid-cols-3 gap-2">
                                <button onClick={() => submitCreativeCriticFeedback.mutate({ projectId, sceneId: scene.id, assetId: asset.id, criticRunId: criticRun?.id, rating: 'UP', categories: [] })} className="rounded-xl bg-[rgba(79,139,214,0.10)] border border-[rgba(79,139,214,0.25)] px-2 py-2 text-xs font-black text-[var(--noc-blue)]">Thumbs up</button>
                                <button onClick={() => submitCreativeCriticFeedback.mutate({ projectId, sceneId: scene.id, assetId: asset.id, criticRunId: criticRun?.id, rating: 'DOWN', categories: [] })} className="rounded-xl bg-[rgba(217,70,168,0.10)] border border-[rgba(217,70,168,0.25)] px-2 py-2 text-xs font-black text-[var(--noc-magenta)]">Thumbs down</button>
                                <button onClick={() => submitCreativeCriticFeedback.mutate({ projectId, sceneId: scene.id, assetId: asset.id, criticRunId: criticRun?.id, rating: 'NEEDS_IMPROVEMENT', categories: ['OTHER'] })} className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.10)] px-2 py-2 text-xs font-black text-[var(--noc-t4)]">Needs work</button>
                              </div>
                            )}
                            {!isR16 && <button onClick={() => toggleCompareAsset(scene, asset.id)} className="rounded-xl bg-[var(--noc-page)] px-3 py-2 text-sm font-black text-white">Compare</button>}
                            {!isR16 && !isActive && <button onClick={() => deleteAsset.mutate({ projectId, assetId: asset.id })} className="rounded-xl border border-[rgba(217,70,168,0.20)] bg-[rgba(217,70,168,0.08)] px-3 py-2 text-sm font-black text-[var(--noc-magenta)]"><Trash2 className="mr-1 inline" size={16} />Remove</button>}
                          </div>
                        </div>
                      </article>
                    );
      ...
      {!isR16 && compareAssets.length === 2 && (
        <div className="mt-6 rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[var(--noc-bar)] p-4 text-white">
  ...
  const renderSequence = () => {
    ...
    if (sequenceQuery.isLoading) return <section className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-8 font-bold text-[var(--noc-t4)]">Loading sequence...</section>;
    if (sequenceQuery.error) return <section className="rounded-2xl border border-[rgba(217,70,168,0.20)] bg-[rgba(217,70,168,0.06)] p-8 font-bold text-[var(--noc-magenta)]">{sequenceQuery.error.message}</section>;
    const data = sequenceQuery.data as SequenceData | undefined;
    if (!data?.sequence) return <section className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-8 font-bold text-[var(--noc-t4)]">No sequence yet.</section>;
    const sequence = data.sequence;
    ...
    return (
      <section className="space-y-5">
        <div className="rounded-2xl bg-[var(--noc-bar)] p-5 text-white">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-5">
            <div className="overflow-hidden rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)]">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[rgba(233,233,237,0.08)] px-4 py-3">
                <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Timeline Editor</p>
                <button onClick={() => selected && restoreSourceScene.mutate({ projectId, sequenceId: sequence.id, storySceneId: selected.storySceneId, afterSequenceSceneId: selected.id })} disabled={!selected || restoreSourceScene.isPending} className="rounded-xl bg-[rgba(79,139,214,0.10)] px-3 py-2 text-xs font-black text-[var(--noc-purple)]">Add Source Scene</button>
              </div>
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Shot {sequenceScene.orderIndex}</p>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2 text-xs font-black">
                          ...
                          <span className="rounded-full bg-[rgba(79,139,214,0.12)] px-2 py-1">{label(sequenceScene.cameraMovement) || 'Camera'}</span>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4">
              <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Runtime Panel</p>
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm font-black">
                <div className="rounded-xl bg-[var(--noc-page)] p-3"><p className="text-[var(--noc-t4)]">Runtime</p><p className="text-2xl">{formatRuntime(data.runtime.totalRuntimeSeconds)}</p></div>
                <div className="rounded-xl bg-[var(--noc-page)] p-3"><p className="text-[var(--noc-t4)]">Shots</p><p className="text-2xl">{data.runtime.activeShotCount}</p></div>
                <div className="rounded-xl bg-[var(--noc-page)] p-3"><p className="text-[var(--noc-t4)]">Avg</p><p className="text-2xl">{data.runtime.averageShotLength}s</p></div>
                <div className="rounded-xl bg-[var(--noc-page)] p-3"><p className="text-[var(--noc-t4)]">Entries</p><p className="text-2xl">{data.runtime.totalShotCount}</p></div>
              </div>
              <div className="mt-3 rounded-xl bg-[rgba(79,139,214,0.10)] p-3 text-sm font-bold text-[var(--noc-t4)]">
                <p>Narration: <span className="text-[var(--noc-t1)]">Not added</span></p>
                <p>Music: <span className="text-[var(--noc-t1)]">Not added</span></p>
                <p>Movie: <span className="text-[var(--noc-t1)]">Not rendered</span></p>
                <p>Render estimate: <span className="text-[var(--noc-t1)]">Available in Movie Builder</span></p>
              </div>
            </div>
            ...
            {selected && (
              <div className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <label className="text-xs font-black uppercase text-[var(--noc-t4)]">Hold<input type="number" min={0} max={10} step={0.5} value={selected.holdDurationSeconds ?? 0} onChange={(event) => updateSequenceEntry(selected, { holdDurationSeconds: Number(event.target.value) })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]" /></label>
                    <label className="text-xs font-black uppercase text-[var(--noc-t4)]">Transition Time<input type="number" min={0} max={10} step={0.5} value={selected.transitionDurationSeconds ?? 0} onChange={(event) => updateSequenceEntry(selected, { transitionDurationSeconds: Number(event.target.value) })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]" /></label>
                  </div>
                  <label className="block text-xs font-black uppercase text-[var(--noc-t4)]">Shot<select value={selected.shotType ?? ''} onChange={(event) => updateSequenceEntry(selected, { shotType: event.target.value || null })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">Choose shot</option>{SHOT_TYPE_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></label>
                  <label className="block text-xs font-black uppercase text-[var(--noc-t4)]">Camera<select value={selected.cameraMovement ?? ''} onChange={(event) => updateSequenceEntry(selected, { cameraMovement: event.target.value || null })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">Choose movement</option>{CAMERA_MOVEMENT_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></label>
                  <div className="grid grid-cols-2 gap-2">
                    <label className="text-xs font-black uppercase text-[var(--noc-t4)]">Speed<select value={selected.cameraSpeed ?? ''} onChange={(event) => updateSequenceEntry(selected, { cameraSpeed: event.target.value || null })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">Speed</option>{CAMERA_SPEED_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></label>
                    <label className="text-xs font-black uppercase text-[var(--noc-t4)]">Multiplier<input type="number" min={0.1} max={4} step={0.1} value={selected.cameraSpeedMultiplier ?? 1} onChange={(event) => updateSequenceEntry(selected, { cameraSpeedMultiplier: Number(event.target.value) })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]" /></label>
                  </div>
                  <label className="block text-xs font-black uppercase text-[var(--noc-t4)]">Transition<select value={selected.transition ?? ''} onChange={(event) => updateSequenceEntry(selected, { transition: event.target.value || null })} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-2 text-sm normal-case text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">Choose transition</option>{TRANSITION_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></label>
                  <label className="block text-xs font-black uppercase text-[var(--noc-t4)]">Notes<textarea value={selected.creativeNotes ?? ''} onChange={(event) => updateSequenceEntry(selected, { creativeNotes: event.target.value })} rows={3} className="mt-1 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 text-sm normal-case text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" placeholder="Creative or editorial notes" /></label>
                </div>
              </div>
            )}

            <div className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4">
              <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Version History</p>
              <div className="mt-3 flex gap-2">
                <input value={versionTitle} onChange={(event) => setVersionTitle(event.target.value)} placeholder={`Version ${sequence.currentVersionNumber + 1}`} className="min-w-0 flex-1 rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] px-3 py-2 text-sm font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
                <button onClick={() => createSequenceVersion.mutate({ projectId, sequenceId: sequence.id, title: versionTitle || undefined })} disabled={createSequenceVersion.isPending} className="rounded-xl bg-[var(--noc-page)] px-3 py-2 text-sm font-black text-white"><Save className="inline" size={15} /></button>
              </div>
              <div className="mt-3 space-y-2">
                {(sequence.versions ?? []).length === 0 ? <p className="rounded-xl border border-dashed border-[rgba(233,233,237,0.10)] p-4 text-sm font-bold text-[var(--noc-t4)]">No saved versions yet.</p> : sequence.versions?.map((version) => (
                  <div key={version.id} className="rounded-xl bg-[var(--noc-page)] p-3">
                    <p className="font-black">v{version.versionNumber} - {version.title}</p>
                    <p className="text-xs font-bold text-[var(--noc-t4)]">{formatRuntime(version.runtimeSeconds)} - {dateLabel(version.createdAt)}</p>
                    <div className="mt-2 flex gap-2">
                      <button onClick={() => restoreSequenceVersion.mutate({ projectId, sequenceId: sequence.id, versionId: version.id })} disabled={restoreSequenceVersion.isPending} className="rounded-lg bg-[rgba(178,90,217,0.10)] border border-[rgba(178,90,217,0.25)] px-3 py-2 text-xs font-black text-[var(--noc-purple)]">Restore</button>
                      <button onClick={() => duplicateSequenceVersion.mutate({ projectId, sequenceId: sequence.id, versionId: version.id })} disabled={duplicateSequenceVersion.isPending} className="rounded-lg bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.10)] px-3 py-2 text-xs font-black text-[var(--noc-t4)]">Duplicate</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4">
              <p className="text-xs font-black uppercase text-[var(--noc-t4)]">Film Blueprint</p>
              <p className="mt-2 text-sm font-bold text-[var(--noc-t4)]">Phase 9B will consume this EDL contract. Provider and rendering settings are intentionally absent.</p>
              <div className="mt-3 rounded-xl bg-[rgba(233,233,237,0.06)] p-3 text-xs font-black text-[var(--noc-t4)]">
  ...
  const renderAudio = () => {
    if (isR16) return null;
    if (audioPlanQuery.isLoading) return <section className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-8 font-bold text-[var(--noc-t4)]">Loading Audio & Performance Plan...</section>;
    if (audioPlanQuery.error) return <section className="rounded-2xl border border-[rgba(217,70,168,0.20)] bg-[rgba(217,70,168,0.06)] p-8 font-bold text-[var(--noc-magenta)]">{audioPlanQuery.error.message}</section>;
      ...
      <section className="space-y-5">
        <div className="rounded-2xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.10)] p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-blue)]">Audio & Performance</p>
              <h2 className="mt-1 text-3xl font-black text-[var(--noc-t1)]">Audio Plan</h2>
            </div>
            <div className="flex items-center gap-4 rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.03)] px-4 py-2">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t5)]">Runtime</p>
                <p className="text-lg font-black text-[var(--noc-t1)]">{runtimeSeconds.toFixed(1)} sec</p>
              </div>
              <div className="h-8 w-px bg-[rgba(233,233,237,0.10)]" />
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t5)]">Version</p>
                <p className="text-lg font-black text-[var(--noc-t1)]">{plan?.currentVersionNumber ?? 0}</p>
              </div>
            </div>
          </div>
          <p className="mt-2 max-w-3xl text-sm font-semibold text-[var(--noc-t4)]">
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="rounded-xl border border-[rgba(79,139,214,0.18)] bg-[rgba(79,139,214,0.08)] p-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-blue)]">Timeline Preview</p>
                  <p className="mt-1 text-sm font-bold text-[var(--noc-t1)]">{safePlayhead.toFixed(1)}s / {runtimeSeconds.toFixed(1)}s</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setAudioPlayheadSeconds(0)}
                    className="inline-flex items-center gap-1 rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.06)] px-3 py-2 text-xs font-black text-[var(--noc-t1)]"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-xl bg-[rgba(178,90,217,0.14)] px-3 py-2 text-xs font-black text-[var(--noc-purple)] disabled:opacity-40"
                  >
            ...
            <div className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Cue Audio Preview</p>
              {selectedCueAudioUrl ? (
                <audio key={selectedCue.id} controls src={selectedCueAudioUrl} className="mt-3 w-full" />
              ) : (
                <p className="mt-3 rounded-lg border border-dashed border-[rgba(233,233,237,0.12)] p-3 text-xs font-bold text-[var(--noc-t5)]">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            {AUDIO_TRACK_TYPES.map((type) => (
              <button key={type} onClick={() => setNewAudioTrackType(type)} className={`rounded-full px-3 py-1.5 text-xs font-black uppercase tracking-wide ${newAudioTrackType === type ? 'bg-[linear-gradient(90deg,#d946a8,#b25ad9)] text-[var(--noc-t1)]' : 'border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] text-[var(--noc-t4)]'}`}>
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-[rgba(233,233,237,0.08)] border border-[rgba(233,233,237,0.10)] px-3 py-1.5 text-xs font-black text-[var(--noc-t1)] disabled:opacity-50"
            >
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="space-y-4 overflow-x-auto rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-5">
            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Timeline</p>
            <div className="min-w-[560px]">
              <div className="relative mb-2 h-5 border-b border-[rgba(233,233,237,0.10)] text-[10px] font-bold text-[var(--noc-t5)]">
                {timeMarks.map((s) => (
                  <span key={s} className="absolute" style={{ left: `${(s / runtimeSeconds) * 100}%` }}>{s}s</span>
                ))}
              </div>
              {tracks.length === 0 && (
                <p className="rounded-xl border border-dashed border-[rgba(233,233,237,0.10)] p-5 text-sm font-bold text-[var(--noc-t4)]">No tracks yet. Add a Narration, Dialogue, Ambience, SFX, or Music track above.</p>
              )}
              <div className="space-y-3">
                {tracks.map((track: any) => (
                  <div key={track.id} className="rounded-xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.03)] p-3">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 text-sm font-black text-[var(--noc-t1)]">
                        <span className="text-[var(--noc-blue)]">{trackIcon(track.type)}</span>
                        {track.name}
                        <span className="rounded-full border border-[rgba(233,233,237,0.10)] px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-[var(--noc-t4)]">{AUDIO_TRACK_TYPE_LABEL[track.type]}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => updateAudioTrack.mutate({ projectId, trackId: track.id, enabled: !track.enabled })}
                          className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${track.enabled ? 'bg-[rgba(79,139,214,0.12)] text-[var(--noc-blue)]' : 'bg-[rgba(233,233,237,0.08)] text-[var(--noc-t5)]'}`}
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-full bg-[rgba(233,233,237,0.08)] px-2 py-1 text-[10px] font-black text-[var(--noc-t1)]"
                        >
                          <Plus size={12} /> Cue
                        </button>
                      </div>
                    ...
                    <div className="relative mt-2 h-12 rounded-lg bg-[rgba(233,233,237,0.04)]">
                      <span
                        className="pointer-events-none absolute bottom-1 top-1 z-10 w-0.5 rounded-full bg-[var(--noc-blue)]"
                      ...
                      {track.cues.map((cue: any) => {
                        ...
                        return (
                          <button
                            ...
                            className={`absolute top-1 h-10 truncate rounded-lg border px-2 text-left text-[10px] font-black leading-4 shadow-sm transition focus:outline-none focus:ring-2 focus:ring-[var(--noc-purple)] ${selectedCueId === cue.id ? 'border-[var(--noc-magenta)] bg-[linear-gradient(90deg,#d946a8,#b25ad9)] text-[var(--noc-t1)]' : 'border-[rgba(79,139,214,0.20)] bg-[rgba(79,139,214,0.15)] text-[var(--noc-blue)] hover:border-[rgba(79,139,214,0.40)] hover:bg-[rgba(79,139,214,0.20)]'}`}
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-[rgba(233,233,237,0.08)] pt-4">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-[var(--noc-blue)] px-4 py-2 text-sm font-black text-white disabled:opacity-50"
              >
                {saveAudioVersion.isPending ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />} Save Version
              </button>
              <span className="text-xs font-bold text-[var(--noc-t5)]">Current version: {plan?.currentVersionNumber ?? 0}</span>
            </div>
          </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-4">
              ...
              {!selectedCue ? (
                ...
                <div className="mt-3 space-y-3">
                  <div className="rounded-xl border border-[rgba(178,90,217,0.20)] bg-[rgba(178,90,217,0.07)] p-3">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <p className="text-sm font-black text-[var(--noc-t1)]">{selectedCue.trackName}</p>
                        <p className="text-xs font-bold text-[var(--noc-t4)]">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg bg-[rgba(233,233,237,0.08)] px-2 py-1 text-[10px] font-black text-[var(--noc-t1)]"
                      >
                        <Play size={11} /> Jump
                      </button>
                    </div>
                    {selectedCueAudioUrl ? (
                      ...
                      <p className="mt-3 text-xs font-bold text-[var(--noc-t4)]">
                  ...
                  {(selectedCue.trackType === 'NARRATION' || selectedCue.trackType === 'DIALOGUE') && (
                    <>
                      <label className="block text-xs font-bold text-[var(--noc-t4)]">Character
                        <select
                      ...
                      {(() => {
                        ...
                        return (
                          <p className="text-xs font-bold text-[var(--noc-t5)]">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <label className="text-xs font-bold text-[var(--noc-t4)]">Duration (s)
                  ...
                  <div className="space-y-3 rounded-xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.03)] p-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="rounded-lg bg-[rgba(233,233,237,0.08)] px-2 py-1 text-[10px] font-black text-[var(--noc-t1)]"
                      >
                        -0.5s
                      </button>
                      <button
                  ...
                  <div className="flex gap-2 pt-2">
                    <button onClick={() => duplicateAudioCue.mutate({ projectId, cueId: selectedCue.id })} className="inline-flex items-center gap-1.5 rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.06)] px-3 py-2 text-xs font-black text-[var(--noc-purple)]"><Copy size={13} /> Duplicate</button>
                    <button onClick={() => { removeAudioCue.mutate({ projectId, cueId: selectedCue.id }); setSelectedCueId(null); }} className="inline-flex items-center gap-1.5 rounded-xl bg-[rgba(217,70,168,0.08)] px-3 py-2 text-xs font-black text-[var(--noc-magenta)]"><Trash2 size={13} /> Remove</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-4">
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Voice Profiles</p>
                <button
              ...
              <div className="mt-3 space-y-1.5">
                {voiceProfiles.length === 0 && <p className="text-xs font-bold text-[var(--noc-t5)]">No voice profiles yet.</p>}
                {voiceProfiles.map((vp: any) => <p key={vp.id} className="text-sm font-bold text-[var(--noc-t1)]">{vp.name}</p>)}
              </div>
            </div>

            <div className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Version History</p>
              <div className="mt-3 space-y-2">
                {versions.length === 0 && <p className="text-xs font-bold text-[var(--noc-t5)]">No versions saved yet.</p>}
                {versions.map((v: any) => (
                  <div key={v.id} className="flex items-center justify-between rounded-lg border border-[rgba(233,233,237,0.08)] p-2">
                    <span className="text-xs font-black text-[var(--noc-t1)]">v{v.versionNumber} — {v.title}</span>
                    <button
                      onClick={() => plan?.id && restoreAudioVersion.mutate({ projectId, planId: plan.id, versionNumber: v.versionNumber })}
                      className="inline-flex items-center gap-1 rounded-full border border-[rgba(233,233,237,0.10)] px-2 py-1 text-[10px] font-black text-[var(--noc-purple)]"
  ...
  const renderFilm = () => {
    if (isR16) return null;
    if (movieBuilderQuery.isLoading) return <section className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-8 font-bold text-[var(--noc-t4)]">Loading Movie Builder...</section>;
    if (movieBuilderQuery.error) return <section className="rounded-2xl border border-[rgba(217,70,168,0.20)] bg-[rgba(217,70,168,0.06)] p-8 font-bold text-[var(--noc-magenta)]">{movieBuilderQuery.error.message}</section>;
    ...
    return (
      <section className="space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-5">
            <div className="overflow-hidden rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)]">
              <div className="border-b border-[rgba(233,233,237,0.08)] px-5 py-4">
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Movie Preview</p>
                <h3 className="mt-1 text-2xl font-black text-[var(--noc-t1)]">{latestReady?.publicUrl ? `Version ${latestReady.versionNumber}` : 'No rendered movie yet'}</h3>
              </div>
              <div className="bg-black p-5">
                <div className="mx-auto aspect-[9/16] max-h-[72vh] overflow-hidden rounded-2xl bg-black">
                  {latestReady?.publicUrl ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-3 text-[var(--noc-t4)]">
              ...
              {latestReady?.publicUrl && (
                <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
                  <div className="text-sm font-bold text-[var(--noc-t4)]">
                    {formatRuntime(latestReady.durationSeconds)} | {latestReady.width}x{latestReady.height} | {latestReady.fps} fps
                  </div>
                  <a href={latestReady.publicUrl} download className="inline-flex items-center gap-2 rounded-xl bg-[rgba(233,233,237,0.08)] border border-[rgba(233,233,237,0.10)] px-4 py-2 text-sm font-black text-[var(--noc-t1)]"><Download size={16} />Download MP4</a>
                </div>
              )}
            </div>

            <div className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-5">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Render History</p>
              <div className="mt-4 space-y-3">
                {history.length === 0 ? <p className="rounded-xl border border-dashed border-[rgba(233,233,237,0.10)] p-5 text-sm font-bold text-[var(--noc-t4)]">No movie renders yet.</p> : history.map((job) => (
                  <article key={job.id} className="rounded-2xl border border-[rgba(233,233,237,0.08)] bg-[rgba(233,233,237,0.04)] p-4">
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Render {job.id.slice(-6)}</p>
                        <h4 className="text-xl font-black text-[var(--noc-t1)]">{label(job.status)}</h4>
                        <p className="mt-1 text-sm font-bold text-[var(--noc-t4)]">{job.currentStage ? label(job.currentStage) : 'Queued'} | {dateLabel(job.createdAt)}</p>
                        {job.errorMessage && <p className="mt-2 rounded-xl border border-[rgba(217,70,168,0.20)] bg-[rgba(217,70,168,0.06)] p-3 text-sm font-bold text-[var(--noc-magenta)]">{job.errorMessage}</p>}
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {job.movieAsset?.publicUrl && !job.movieAsset.isCurrent && <button onClick={() => setCurrentMovie.mutate({ projectId, movieAssetId: job.movieAsset!.id })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.06)] px-3 py-2 text-xs font-black text-[var(--noc-purple)]">Use This</button>}
                        {['FAILED', 'CANCELLED'].includes(job.status) && <button onClick={() => retryMovieRender.mutate({ projectId, renderJobId: job.id })} disabled={retryMovieRender.isPending} className="rounded-xl bg-[rgba(79,139,214,0.10)] px-3 py-2 text-xs font-black text-[var(--noc-purple)]">Retry</button>}
                        {['QUEUED', 'PREPARING'].includes(job.status) && <button onClick={() => cancelMovieRender.mutate({ projectId, renderJobId: job.id })} disabled={cancelMovieRender.isPending} className="rounded-xl bg-[rgba(217,70,168,0.08)] px-3 py-2 text-xs font-black text-[var(--noc-magenta)]">Cancel</button>}
                      </div>
                    </div>
                    <div className="mt-3 h-2 overflow-hidden rounded-full bg-[rgba(233,233,237,0.08)]">
                      <div className="h-full rounded-full bg-[var(--noc-blue)]" style={{ width: `${Math.min(100, Math.max(0, job.progressPercent ?? 0))}%` }} />
                    </div>
                  </article>
                ))}
              </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Preflight</p>
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm font-black">
                <div className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.08)] p-3"><p className="text-[var(--noc-t4)]">Shots</p><p className="text-2xl text-[var(--noc-t1)]">{readiness?.shotCount ?? 0}</p></div>
                <div className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.08)] p-3"><p className="text-[var(--noc-t4)]">Runtime</p><p className="text-2xl text-[var(--noc-t1)]">{formatRuntime(readiness?.runtimeSeconds ?? 0)}</p></div>
                <div className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.08)] p-3"><p className="text-[var(--noc-t4)]">Output</p><p className="text-2xl text-[var(--noc-t1)]">9:16</p></div>
                <div className="rounded-xl bg-[rgba(233,233,237,0.04)] border border-[rgba(233,233,237,0.08)] p-3"><p className="text-[var(--noc-t4)]">Credits</p><p className="text-2xl text-[var(--noc-t1)]">{data?.creditCost ?? 0}</p></div>
              </div>
              <div className={`mt-3 rounded-xl p-3 text-sm font-bold ${readiness?.ready ? 'bg-[rgba(79,139,214,0.12)] text-[var(--noc-blue)]' : 'bg-[rgba(217,70,168,0.08)] text-[var(--noc-magenta)]'}`}>
                {readiness?.ready ? 'Ready to render from selected pictures.' : 'Fix the sequence picture selections before rendering.'}
              </div>
              {(readiness?.warnings ?? []).length > 0 && <div className="mt-3 space-y-2">{readiness.warnings.map((warning: string) => <p key={warning} className="rounded-xl bg-[rgba(255,207,74,0.08)] p-3 text-xs font-black text-[var(--noc-t2)]">{warning}</p>)}</div>}
            </div>

            <div className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Render Plan</p>
              <div className="mt-3 space-y-2 text-sm font-bold text-[var(--noc-t4)]">
                <p>Renderer: <span className="text-[var(--noc-t1)]">{data?.renderPlan?.rendererVersion ?? 'Not ready'}</span></p>
                <p>Hash: <span className="break-all text-[var(--noc-t1)]">{data?.renderPlanHash ?? 'No render plan'}</span></p>
                <p>Video: <span className="text-[var(--noc-t1)]">720x1280 MP4, 30 fps</span></p>
                <p>Audio: <span className="text-[var(--noc-t1)]">None</span></p>
              </div>
            </div>
          </aside>
        </div>
...
{gatedVisibleTabs.map((item) => (
        ...
        <div className="mt-6">
          ...
          {tab === 'film' && renderFilm()}
          {tab === 'storybook' && <section className="rounded-2xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-8 text-center"><BookOpen className="mx-auto text-[var(--noc-purple)]" size={48} /><h2 className="mt-3 text-3xl font-black text-[var(--noc-t1)]">{isR16 ? 'Read your book' : 'Storybook'}</h2><p className="mt-2 font-semibold text-[var(--noc-t4)]">{isR16 ? 'Open the book with your chosen pictures.' : 'Storybook uses the active image for each scene, then latest image as fallback.'}</p><Link href={`/story-playground/${projectId}/storybook`} className="mt-5 inline-block rounded-xl bg-[linear-gradient(90deg,#d946a8,#b25ad9)] px-5 py-3 font-black text-[var(--noc-t1)]">{isR16 ? 'Read Book' : 'Open Storybook'}</Link></section>}
        </div>
      </main>

      {editingCharacter && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 px-4">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-[var(--noc-page)] border border-[rgba(233,233,237,0.10)] p-5">
            <div className="mb-5 flex items-center justify-between">
              ...
              <button onClick={() => setEditingCharacter(null)} className="rounded-lg p-1 text-[var(--noc-t5)] hover:text-[var(--noc-t1)]"><X size={20} /></button>
            </div>

            {/* Identity */}
            <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Identity</p>
            <div className="grid gap-3 md:grid-cols-2">
              <input value={characterForm.name} onChange={(e) => setCharacterForm({ ...characterForm, name: e.target.value })} placeholder="Name" className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              <input value={characterForm.role} onChange={(e) => setCharacterForm({ ...characterForm, role: e.target.value })} placeholder={isR16 ? 'Role (hero, sidekick…)' : 'Role'} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              <input value={characterForm.species} onChange={(e) => setCharacterForm({ ...characterForm, species: e.target.value })} placeholder={isR16 ? 'Species (human, dragon…)' : 'Species'} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              <input value={characterForm.ageDescription} onChange={(e) => setCharacterForm({ ...characterForm, ageDescription: e.target.value })} placeholder={isR16 ? 'Age (young, adult…)' : 'Age description'} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              <input value={characterForm.gender} onChange={(e) => setCharacterForm({ ...characterForm, gender: e.target.value })} placeholder={isR16 ? 'Gender (optional)' : 'Gender (optional)'} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
            </div>

            {/* Appearance */}
            <div className="mt-5 border-t border-[rgba(233,233,237,0.08)] pt-4">
              <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Appearance</p>
              <textarea value={characterForm.visualDescription} onChange={(e) => setCharacterForm({ ...characterForm, visualDescription: e.target.value })} placeholder={isR16 ? 'How does this character look? Describe their clothes, colours, face…' : 'Visual description — clothing, colours, face, distinctive features'} rows={4} className="w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
            </div>

            {/* Personality */}
            <div className="mt-5 border-t border-[rgba(233,233,237,0.08)] pt-4">
              <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Personality</p>
              <div className="flex flex-wrap gap-2">
                {PERSONALITY_OPTIONS.map((trait) => (
                  <button key={trait} onClick={() => setCharacterForm((current) => ({ ...current, personalityTraits: current.personalityTraits.includes(trait) ? current.personalityTraits.filter((item) => item !== trait) : [...current.personalityTraits, trait] }))} className={`rounded-full px-3 py-1.5 text-sm font-black transition-colors ${characterForm.personalityTraits.includes(trait) ? 'bg-[rgba(79,139,214,0.15)] text-[var(--noc-blue)] border border-[rgba(79,139,214,0.4)]' : 'border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] text-[var(--noc-t4)] hover:border-[rgba(233,233,237,0.20)]'}`}>{label(trait)}</button>
                ))}
              </div>
            </div>
            ...
            {/* Psychology */}
            <div className="mt-5 border-t border-[rgba(233,233,237,0.08)] pt-4">
              <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Psychology</p>
              <div className="grid gap-3 md:grid-cols-3">
                <select value={characterForm.motivation} onChange={(e) => setCharacterForm({ ...characterForm, motivation: e.target.value })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">{isR16 ? 'What they want' : 'Motivation'}</option>{MOTIVATION_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select>
                <select value={characterForm.fear} onChange={(e) => setCharacterForm({ ...characterForm, fear: e.target.value })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">{isR16 ? 'What they fear' : 'Fear'}</option>{FEAR_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select>
                <select value={characterForm.goal} onChange={(e) => setCharacterForm({ ...characterForm, goal: e.target.value })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">{isR16 ? 'Their goal' : 'Goal'}</option>{GOAL_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select>
              </div>
            </div>

            {/* Behavior */}
            <div className="mt-5 border-t border-[rgba(233,233,237,0.08)] pt-4">
              <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Behavior</p>
              <div className="grid gap-3 md:grid-cols-2">
                <select value={characterForm.walkingStyle} onChange={(e) => setCharacterForm({ ...characterForm, walkingStyle: e.target.value })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="">{isR16 ? 'How they walk' : 'Walking style'}</option>{WALKING_OPTIONS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select>
              </div>
            </div>

            {/* Relationships */}
            <div className="mt-5 border-t border-[rgba(233,233,237,0.08)] pt-4">
              <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Relationships</p>
              <div className="grid gap-3 md:grid-cols-3">
                <input value={characterForm.relationshipTargetName} onChange={(e) => setCharacterForm({ ...characterForm, relationshipTargetName: e.target.value })} placeholder={isR16 ? 'Friend\'s name' : 'Relationship target'} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
                <select value={characterForm.relationshipType} onChange={(e) => setCharacterForm({ ...characterForm, relationshipType: e.target.value })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="FRIEND">{isR16 ? 'Friend' : 'Friend'}</option><option value="RIVAL">Rival</option><option value="MENTOR">Mentor</option><option value="ENEMY">Enemy</option><option value="ROMANTIC">Romantic</option><option value="FAMILY">Family</option></select>
                <select value={characterForm.relationshipStrength} onChange={(e) => setCharacterForm({ ...characterForm, relationshipStrength: e.target.value })} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-purple)]"><option value="VERY_CLOSE">{isR16 ? 'Very close' : 'Very close'}</option><option value="CLOSE">Close</option><option value="NEUTRAL">Neutral</option><option value="DISTANT">Distant</option><option value="ESTRANGED">Estranged</option></select>
              </div>
            </div>
            ...
            {!isR16 && (
              <div className="mt-5 border-t border-[rgba(233,233,237,0.08)] pt-4">
                <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[var(--noc-t4)]">Evolution</p>
                <div className="grid gap-3 md:grid-cols-2">
                  <input value={characterForm.evolutionStage} onChange={(e) => setCharacterForm({ ...characterForm, evolutionStage: e.target.value })} placeholder="Stage (e.g. Act 2 breakthrough)" className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
                  <input value={characterForm.evolutionSceneOrder} onChange={(e) => setCharacterForm({ ...characterForm, evolutionSceneOrder: e.target.value })} placeholder="Scene order (e.g. 3)" className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
                  <textarea value={characterForm.evolutionNotes} onChange={(e) => setCharacterForm({ ...characterForm, evolutionNotes: e.target.value })} placeholder="Evolution notes" rows={2} className="md:col-span-2 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
                </div>
              </div>
            )}

            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setEditingCharacter(null)} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.06)] px-5 py-3 font-black text-[var(--noc-t1)]">Cancel</button>
      ...
      {editingScene && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 px-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-[var(--noc-page)] border border-[rgba(233,233,237,0.10)] p-5">
            ...
            <input value={sceneForm.title} onChange={(e) => setSceneForm({ ...sceneForm, title: e.target.value })} className="w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
            <textarea value={sceneForm.description} onChange={(e) => setSceneForm({ ...sceneForm, description: e.target.value })} rows={4} className="mt-3 w-full rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <input value={sceneForm.locationType} onChange={(e) => setSceneForm({ ...sceneForm, locationType: e.target.value })} placeholder="Place" className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              <input value={sceneForm.indoorOutdoor} onChange={(e) => setSceneForm({ ...sceneForm, indoorOutdoor: e.target.value })} placeholder="Indoor/outdoor" className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              <input value={sceneForm.mood} onChange={(e) => setSceneForm({ ...sceneForm, mood: e.target.value })} placeholder="Mood" className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]" />
              {Object.entries(DIRECTOR_OPTIONS).map(([key, values]) => <select key={key} value={(sceneForm as any)[key]} onChange={(e) => setSceneForm({ ...sceneForm, [key]: e.target.value } as any)} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.04)] p-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t5)] outline-none focus:border-[var(--noc-purple)]"><option value="">{label(key)}</option>{values.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select>)}
            </div>
            <div className="mt-5 flex justify-end gap-3"><button onClick={() => setEditingScene(null)} className="rounded-xl border border-[rgba(233,233,237,0.10)] bg-[rgba(233,233,237,0.06)] px-5 py-3 font-black text-[var(--noc-t1)]">Cancel</button><button onClick={saveScene} className="rounded-xl bg-[linear-gradient(90deg,#d946a8,#b25ad9,#4f8bd6)] px-5 py-3 font-black text-white">Save</button></div>
          </div>
        </div>
      )}
      ...
      {previewAsset && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 px-4">
          <div className="max-h-[92vh] max-w-3xl overflow-y-auto rounded-2xl bg-[var(--noc-page)] border border-[rgba(233,233,237,0.10)] p-4">
