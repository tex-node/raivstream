  ...
  if (isLoaded && !isSignedIn) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center gap-4">
        <Navbar />
        <p className="text-white/60 text-lg">Sign in to use Story Studio</p>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {projects?.map((item: { id: string; title: string; _count: { shots: number } }) => (
              ...
              >
                ...
                <span className="text-white/35">{item._count.shots} shots</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr_360px] gap-5">
          <section className="space-y-4">
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-pink-500/60"
                />
                <input
                  value={logline}
                ...
                <textarea
                  ...
                  rows={7}
                  className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none resize-none focus:border-pink-500/60"
                />
                <textarea
                  value={visualStyle}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    className="rounded-lg bg-pink-500 px-3 py-2 text-sm font-semibold hover:bg-pink-600 disabled:opacity-40"
                  >
                    Save
                  </button>
                </div>
                <button
                  ...
                  className="w-full rounded-lg border border-emerald-500/30 bg-emerald-500/15 px-3 py-2 text-sm font-semibold text-emerald-200 hover:bg-emerald-500/20 disabled:opacity-40"
                >
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="space-y-2">
                <input value={characterForm.name} onChange={(e) => setCharacterForm({ ...characterForm, name: e.target.value })} placeholder="Name" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <input value={characterForm.role} onChange={(e) => setCharacterForm({ ...characterForm, role: e.target.value })} placeholder="Role" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <textarea value={characterForm.description} onChange={(e) => setCharacterForm({ ...characterForm, description: e.target.value })} placeholder="Backstory and function in the story" rows={3} className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none resize-none" />
                <textarea value={characterForm.visualTraits} onChange={(e) => setCharacterForm({ ...characterForm, visualTraits: e.target.value })} placeholder="Consistent visual traits, wardrobe, age, silhouette" rows={2} className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none resize-none" />
                <input value={characterForm.referenceUrl} onChange={(e) => setCharacterForm({ ...characterForm, referenceUrl: e.target.value })} placeholder="Reference image URL" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <button
                ...
                <div className="space-y-2 pt-2">
                  {project?.characters.map((character: { id: string; name: string; description: string; visualTraits: string | null }) => (
                    <div key={character.id} className="rounded-lg border border-white/10 bg-black/25 p-3">
                      <p className="font-semibold text-sm">{character.name}</p>
                      <p className="text-xs text-white/45 line-clamp-2">{character.visualTraits || character.description}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="text-xs text-white/35">{project?.shots.length ?? 0} shots</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              {project?.shots.map((shot: Shot) => (
                ...
                >
                  <div className="aspect-[9/16] rounded-lg bg-white/5 mb-3 overflow-hidden flex items-center justify-center">
                    {shot.assetUrl ? (
                      ...
                      <span className="text-white/20 text-2xl">{shot.position}</span>
                    )}
                  </div>
                  <p className="font-semibold text-sm line-clamp-1">{shot.title}</p>
                  <p className="text-xs text-white/45 line-clamp-3 mt-1">{shot.beat}</p>
                </button>
              ))}
              {!project?.shots.length && (
                <div className="md:col-span-2 xl:col-span-3 flex min-h-96 items-center justify-center rounded-xl border border-dashed border-white/10 text-center text-sm text-white/35 px-6">
          ...
          <section className="space-y-4">
            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="space-y-2">
                <input value={environmentForm.name} onChange={(e) => setEnvironmentForm({ ...environmentForm, name: e.target.value })} placeholder="Environment name" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <textarea value={environmentForm.description} onChange={(e) => setEnvironmentForm({ ...environmentForm, description: e.target.value })} placeholder="Location, props, textures, production details" rows={3} className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none resize-none" />
                <input value={environmentForm.mood} onChange={(e) => setEnvironmentForm({ ...environmentForm, mood: e.target.value })} placeholder="Mood" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <input value={environmentForm.lighting} onChange={(e) => setEnvironmentForm({ ...environmentForm, lighting: e.target.value })} placeholder="Lighting" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <input value={environmentForm.referenceUrl} onChange={(e) => setEnvironmentForm({ ...environmentForm, referenceUrl: e.target.value })} placeholder="Reference image URL" className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" />
                <button
                ...
                <div className="space-y-2 pt-2">
                  {project?.environments.map((environment: { id: string; name: string; description: string }) => (
                    <div key={environment.id} className="rounded-lg border border-white/10 bg-black/25 p-3">
                      <p className="font-semibold text-sm">{environment.name}</p>
                      <p className="text-xs text-white/45 line-clamp-2">{environment.description}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {selectedShot && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-white/40">Image prompt</label>
                    <textarea readOnly value={selectedShot.imagePrompt} rows={5} className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white/70 outline-none resize-none" />
                  </div>
                  <div>
                    <label className="text-xs text-white/40">Video prompt</label>
                    <textarea readOnly value={selectedShot.videoPrompt} rows={5} className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white/70 outline-none resize-none" />
                  </div>
                  <input
                    value={assetUrl}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => openGenerate(selectedShot, 'image')} className="rounded-lg bg-pink-500 px-3 py-2 text-sm font-semibold hover:bg-pink-600">
                      Generate Image
                    </button>
                    <button onClick={() => openGenerate(selectedShot, 'video')} className="rounded-lg bg-violet-500 px-3 py-2 text-sm font-semibold hover:bg-violet-600">
                      Generate Video
                    </button>
                  </div>
                  {selectedShot.assetUrl && (
                    <a href={selectedShot.assetUrl} target="_blank" rel="noreferrer" className="block truncate text-xs text-pink-300 hover:text-pink-200">
