      ...
      {data && (
        <div className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-xs font-black uppercase text-white/40">Movie Render Credits</p><p className={`mt-2 text-3xl font-black ${data.movieRenderRateConfigured ? 'text-green-400' : 'text-red-400'}`}>{data.movieRenderRateConfigured ? `${data.movieRenderCreditCost} cr` : 'Not Set'}</p><p className={`mt-1 text-xs font-bold ${data.movieRenderConfigurationHealthy ? 'text-green-400/70' : 'text-red-400/70'}`}>{data.movieRenderConfigurationHealthy ? 'Configured' : 'Action required'}</p></div>
