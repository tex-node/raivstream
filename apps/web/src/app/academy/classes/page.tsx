  ...
  return (
    <AcademyShell title="Classes" subtitle="Create cohorts, share invite codes, and manage students.">
      <div className="grid gap-5 lg:grid-cols-[.85fr_1.15fr]">
        ...
        <Card>
          <p className="text-sm font-black uppercase text-[var(--noc-blue)]">New Class</p>
          ...
          <form onSubmit={(event) => { event.preventDefault(); if (selectedCourseId && classTitle.trim()) createClass.mutate({ courseId: selectedCourseId, title: classTitle }); }} className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
            <select value={selectedCourseId} onChange={(event) => setSelectedCourseId(event.target.value)} className="rounded-xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] px-4 py-3 font-bold text-[var(--noc-t1)] outline-none focus:border-[var(--noc-blue)]">
              <option value="">Choose course</option>
              {courseList.map((course: any) => <option key={course.id} value={course.id}>{course.title}</option>)}
            </select>
            <input value={classTitle} onChange={(event) => setClassTitle(event.target.value)} placeholder="AI Filmmaking - July Cohort" className="rounded-xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] px-4 py-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t6)] outline-none focus:border-[var(--noc-blue)]" />
            <button disabled={createClass.isPending} className="rounded-xl bg-[var(--noc-blue)] px-4 py-3 font-black text-white disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--noc-blue)]/60">Create</button>
          </form>
      ...
      <Card className="mt-5">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {classes.map((klass: any) => (
            <Link key={klass.id} href={`/academy/classes/${klass.id}`} className="rounded-2xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] p-4 hover:ring-2 hover:ring-[var(--noc-blue)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--noc-blue)]/60">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full bg-[var(--noc-card)] px-3 py-1 text-xs font-black">{statusLabel(klass.status)}</span>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs font-black">
                <span className="rounded-xl bg-[var(--noc-card)] p-2">{klass.memberships.length} people</span>
                <span className="rounded-xl bg-[var(--noc-card)] p-2">{klass.assignments.length} tasks</span>
                <span className="rounded-xl bg-[var(--noc-card)] p-2">{klass.inviteCode}</span>
              </div>
