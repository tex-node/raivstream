  ...
  return (
    <AcademyShell title={assignment.title} subtitle={`${data.student.displayName} · ${statusLabel(data.status)}`}>
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <div className="space-y-5">
          <Card>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-4">
              <div className="rounded-xl bg-[var(--noc-card)] p-3"><p className="text-xs font-black uppercase text-[var(--noc-t4)]">Characters</p><p className="text-2xl font-black">{snapshot.characterCount ?? project?.characterMemory?.length ?? 0}</p></div>
              <div className="rounded-xl bg-[var(--noc-card)] p-3"><p className="text-xs font-black uppercase text-[var(--noc-t4)]">Scenes</p><p className="text-2xl font-black">{snapshot.sceneCount ?? project?.sceneSeeds?.length ?? 0}</p></div>
              <div className="rounded-xl bg-[var(--noc-card)] p-3"><p className="text-xs font-black uppercase text-[var(--noc-t4)]">Storybook</p><p className="text-2xl font-black">{snapshot.storybookReady ? 'Ready' : 'Open'}</p></div>
              <div className="rounded-xl bg-[var(--noc-card)] p-3"><p className="text-xs font-black uppercase text-[var(--noc-t4)]">Submitted</p><p className="text-sm font-black">{dateLabel(data.submittedAt)}</p></div>
            </div>
            {project && (
              <div className="mt-5 flex flex-wrap gap-3">
                <Link href={`/story-playground/${project.id}`} className="rounded-xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] px-4 py-3 font-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--noc-blue)]/60">Open Workspace Read-Only</Link>
          ...
          <Card>
            ...
            {reviewer && (
              <form onSubmit={(event) => { event.preventDefault(); if (comment.trim()) addComment.mutate({ submissionId, targetType: 'SUBMISSION', body: comment }); }} className="mt-4 grid gap-3">
                <textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Add feedback for the student." className="min-h-28 rounded-xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] px-4 py-3 font-semibold text-[var(--noc-t1)] placeholder:text-[var(--noc-t6)] outline-none focus:border-[var(--noc-blue)]" />
            ...
            <div className="mt-4 grid gap-3">
              {(data.comments ?? []).map((item: any) => (
                <div key={item.id} className="rounded-xl bg-[var(--noc-card)] p-4">
        ...
        <div className="space-y-5">
          <Card>
            <p className="text-sm font-black uppercase text-[var(--noc-blue)]">Instructor Review</p>
            {reviewer ? (
              <div className="mt-4 grid gap-3">
                <textarea value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="Overall feedback summary." className="min-h-28 rounded-xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] px-4 py-3 font-semibold text-[var(--noc-t1)] placeholder:text-[var(--noc-t6)] outline-none focus:border-[var(--noc-blue)]" />
                <input value={score} onChange={(event) => setScore(event.target.value)} placeholder="Score, e.g. 88" className="rounded-xl border border-[var(--noc-hairline)] bg-[var(--noc-card)] px-4 py-3 font-bold text-[var(--noc-t1)] placeholder:text-[var(--noc-t6)] outline-none focus:border-[var(--noc-blue)]" />
                <div className="grid gap-2">
                  ...
                  <button onClick={() => submitReview('APPROVED')} className="rounded-xl bg-[var(--noc-blue)] px-4 py-3 font-black text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--noc-blue)]/60">Approve</button>
