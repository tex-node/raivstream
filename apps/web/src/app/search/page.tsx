    ...
    <div className="max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto pt-20 px-4 pb-16">
      ...
      {!isLoading && users && users.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xs font-semibold text-[var(--noc-t5)] uppercase tracking-wider mb-3">People</h2>
          <div className="space-y-2">
            {users.map((user) => (
              <Link
                key={user.id}
                href={`/${user.username}`}
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-[var(--noc-card)] transition-colors"
              >
                <div className="w-11 h-11 rounded-full overflow-hidden flex-shrink-0 bg-[var(--noc-magenta)] flex items-center justify-center">
                  {user.avatarUrl ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {user.verified && (
                      <svg className="w-4 h-4 text-[var(--noc-blue)] flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
                      </svg>
                    )}
                  </div>
                  <p className="text-xs text-[var(--noc-t5)]">@{user.username} · {user.followerCount.toLocaleString()} followers</p>
                  {user.bio && <p className="text-xs text-[var(--noc-t4)] mt-0.5 truncate">{user.bio}</p>}
                </div>
      ...
      {!isLoading && videos.length === 0 && query && (!users || users.length === 0) && (
        <div className="text-center py-16 text-[var(--noc-t5)]">
      ...
      {videos.length > 0 && (
        <section>
          {users && users.length > 0 && (
            <h2 className="text-xs font-semibold text-[var(--noc-t5)] uppercase tracking-wider mb-3">Videos</h2>
          )}
              ...
              <Link key={video.id} href={`/v/${video.id}`} className="group">
                <div className="relative aspect-[9/16] bg-[var(--noc-page)] rounded-xl overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    ...
                    <p className="text-[var(--noc-t4)] text-xs mt-1">@{video.creator.username}</p>
                    <div className="flex items-center gap-2 mt-1.5 text-[var(--noc-t4)] text-xs">
          ...
          {hasNextPage && (
            <button
              ...
              className="w-full mt-8 py-3 rounded-xl border border-[var(--noc-hairline)] hover:border-[rgba(233,233,237,0.20)] text-sm text-[var(--noc-t4)] transition-colors"
            >
