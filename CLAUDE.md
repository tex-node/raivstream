...
- **Story Playground Creative Critic (Phase 8B)**: post-generation art-director review loop with `CreativeCriticRun`, `CreativeCriticFeedback`, structured scores, improvement plans, asset creative lifecycle, non-R16 review/regenerate/approve/reject controls, R16-safe filtering, and `/admin/prompt-quality` critic inspection
- **Creative Critic Runtime Safety (Phase 8B.1)**: API Vitest harness, retry/idempotency/credit policy helpers, shared Storybook asset-selection helper, R16 API asset sanitization, admin critic runtime counters, structured critic/retry logs, and tests for score decisions, retry caps, R16 filtering, storybook fallback, engine success/failure, and unavailable critic behavior
- **Creative Critic Staging Qualification (Phase 8B.2)**: isolated VPS staging on `/root/raivstream-phase8b2-staging` with dedicated Postgres/PM2, real RunPod/R2/OpenAI smoke for `A dog going to school`, retry idempotency, credit-deduction verification, R16/admin checks, analytics verification, and a Storybook fix preventing creativeStatus `REJECTED` assets from being selected; production rollout was later verified.
- **Story Sequence Workspace (Phase 9A — PRODUCTION COMPLETE)**: additive `StorySequence`, `StorySequenceScene`, and `SequenceVersion` models; non-R16 Sequence tab; canonical edit decision list and Film Blueprint serialization; reorder/duplicate/disable/remove/restore timeline entries; independent asset selection; shot, camera, speed, transition, duration, hold, zoom, and notes planning; timed still-image animatic preview; explicit version save/restore/duplicate; `/admin/sequence` aggregate insights. R16 hides Sequence entirely. No Movie Builder, FFmpeg stitching, image-to-video generation, narration, read-aloud, publishing, marketplace, AI Film Mentor, or Academy expansion.
- **Movie Builder Foundation (Phase 9B.1A — STAGING REQUALIFIED, PRODUCTION RELEASE PENDING)**: additive `MovieRenderJob`, `MovieAsset`, and `MovieRenderEvent` models; deterministic `movieRenderPlanning` helpers that consume the Phase 9A Film Blueprint; server-side `movieRenderWorker` abstraction for FFmpeg still-image segment rendering, camera motion simulation, MP4 assembly, FFprobe verification, R2 upload, render events, and refund-on-failure; non-R16 Story Workspace `Film` tab with preflight, Build Movie, progress polling, preview/download, render history, retry/cancel, and current movie selection; `/admin/movie-renders` diagnostics. Phase 9B.1A fixed the runtime NO-GO: xfade offsets now use `assembledDuration - transitionDuration`, still segments render exact frame counts, FFprobe validation is required before READY, and duration mismatch fails with `OUTPUT_DURATION_MISMATCH`. Staging real FFmpeg cases passed with zero delta for cuts-only 12s, one-dissolve 10s, multiple-dissolve 12s, mixed-transition 16s, and original R2 smoke 12s. Renderer version is `phase-9b1a-v2`. R16 hides Film and movie render APIs. No AI video providers, TTS, soundtrack, subtitles, publishing, marketplace, read-aloud, or Academy expansion were added.
...
- **Audio & Performance Layer (Phase 9B.2 — STAGING-QUALIFIED, PRODUCTION RELEASE PENDING)**: additive `AudioPerformancePlan`, `AudioTrack`, `AudioCue`, `VoiceProfile`, `AudioPlanVersion`, and `AudioAsset` models (migration `20260827120000_audio_performance_phase9b2`); five independently-editable track types (Narration, Dialogue, Ambience, SFX, Music); cue timing anchored to a new `computeCanonicalShotTimeline()` derived from Film Blueprint screen time, never from FFmpeg xfade render handles; deterministic provider-neutral Audio Blueprint (`audioPlanning.ts`) versioned with the same monotonic `max+1` pattern as Sequence versions (no version-number reuse); `MovieRenderJob` extended with nullable `audioBlueprintSnapshot`/`audioPlanVersionId` for immutable per-render snapshots; `combineRenderHash()` invalidates render reuse when audio changes while leaving every pre-existing silent-film hash unchanged; real FFmpeg audio mixing + mux (`audioMixing.ts`) extending, not replacing, the Phase 9B.1 silent-video pipeline; FFprobe READY gate extended (not weakened) with an audio-specific probe that fails closed (`OUTPUT_AUDIO_STREAM_MISSING`/`OUTPUT_AUDIO_VERIFICATION_FAILED`) if expected audio wasn't produced; non-R16 Story Workspace `Audio` tab between Sequence and Film; server-side `assertSequenceAllowed` guard added to all Audio procedures (R16 hides it client-side too); 15 new tRPC procedures; 36 new tests (83/83 total). Full staging qualification in `docs/operations/phase-9b2-staging-qualification.md`, architecture in `docs/architecture/audio-performance-layer.md`, ADR in `docs/adr/phase-9b2-audio-performance.md`. No TTS/voice generation provider, voice cloning, lip sync, subtitle editor, dubbing, AI music generation, publishing, or marketplace were added.
- **Phase 9B.2B checkpoints (PASSED)**: fixed a real bug where a short/late-starting audio cue could truncate the final movie via `-shortest` muxing — `buildMixFilterGraph` now always force-pads/trims the mix to the exact canonical runtime, proven at 0s delta across 8 required scenarios with real ffmpeg. Added `MovieRenderJob.audioBlueprintHash` (migration `20260827180000_audio_blueprint_hash_phase9b2b`), cross-project `AudioAsset` ownership checks (`assertAudioAssetOwnership` write-time + `projectId`-scoped worker query), `getPublicUrlForKey`-derived (never-signed) asset URLs, and an honest "Audio source: Not generated yet" preflight warning (`summarizeUnmaterializedSpeechCues`, WARNING not BLOCKER) plus per-cue indicator — no fake "Generate Voice" control exists. Render idempotency-including-audio-identity and version numbering (`1,2,3`, never `1,2,2`) proven live against real Postgres via `appRouter.createCaller`. 97/97 tests. See `docs/operations/phase-9b2-pure-rendering-checkpoint.md` and `docs/operations/phase-9b2b-persistence-checkpoint.md`.

**Still to build / verify:**
- Controlled Phase 9B.2 production release from the isolated worktree (`C:\Raiv\raivstream-phase9b2-audio`, branch `codex/phase-9b2-audio-performance`): production DB backup/checksum, additive migration deploy, API/web gates, real FFmpeg audio mix+mux smoke, browser signed-in Audio tab QA, admin diagnostics, R16 hiding, and production health preservation.
- Controlled Phase 9B.1 production release from the isolated worktree, including fresh production DB backup/checksum, migration deploy, API/web gates, FFmpeg/R2 render smoke, browser signed-in Film tab QA, admin runtime diagnostics, R16 hiding, and production health preservation.
